...
static const uint8_t InvertLoopTable[]={
	  0,   5,   6,   7,   8,  10,  11, 13,
...
{
	uint16_t sample, period, effect, operiod;
	uint16_t curnote, arpnote;
	uint8_t effect_op;
	uint8_t effect_param,effect_param_l,effect_param_h;
	uint16_t enable_nxt_smp;
	...
	switch ( effect_op )
	{
		...
		case EFFECT_SET_OFFSET:
			...
			*/
			...
			cptr->samppos = ( ( ((uint16_t)effect_param_h) << 12) + ( (((uint16_t)effect_param_l) << 8) ) ) << 10;
		...
		case EFFECT_PATTERN_BREAK:
			...
			*/
			...
			mod->patternpos = ( ((uint16_t)(effect_param_h) * 10) + effect_param_l );
		...
		case 0xF:
			...
			*/
			...
			if( effect_param )
			{
				...
				mod->patternticksem = ( ( mod->playrate * 5 ) / ( (uint32_t)mod->bpm * 2 ) );
#endif
	...
	if( modmemory )
	{
		if( modctx )
		{
			...
			for (i = 0, sptr = modctx->song.samples; i <31; i++, sptr++)
			{
				...
				modctx->sampledata[i] = (int8_t*)modmemory;
			...
			modctx->patternticksem = ( ( modctx->playrate * 5 ) / ( (uint32_t)modctx->bpm * 2 ) );
#endif
...
/* More than just filling a buffer, this appear to be the actual mod processng code*/
	...
	if( modctx && outbuffer )
	{
		if(modctx->mod_loaded)
		{
			...
			for (i = 0; i < nbsample; i++)
			{
					...
					if( cptr->period != 0 )
					{
						...
						if( trkbuf && !state_remaining_steps )
						{
							if( trkbuf->nb_of_state < trkbuf->nb_max_of_state )
							{
								...
								if(cptr->sampinc)
									trkbuf->track_state_buf[trkbuf->nb_of_state].tracks[j].cur_period = (uint16_t)(modctx->sampleticksconst / cptr->sampinc);
								else
				...
				*outbuffer++ = ( r >> 8 ) + 127;
		#else
				*outbuffer++ = (l >> 8)*(modctx->volume)>>8;
				*outbuffer++ = (r >> 8) *(modctx->volume) >> 8;
		#endif
